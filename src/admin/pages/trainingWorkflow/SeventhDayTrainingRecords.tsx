...
/* ── Main component ── */
export const SeventhDayTrainingRecords: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      list.forEach((schedule: any) => {
        ...
        if (!day7Session) {
          console.log(`   ❌ No Day 7 session found for batch ${schedule.batch_no}`);
          return;
        }
        
        console.log(`   ✅ Found Day 7 session: ID=${day7Session.id}, date=${day7Session.date}`);
        
        rows.push({
          key: `${schedule.id}-${day7Session.id}`,
          ...
          date: day7Session.date || '',
          media: Array.isArray(day7Session.media_files) ? day7Session.media_files : [],
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <Text style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 4 }}>
            Training Workflow /&nbsp;
            <span style={{ color: "#2563eb", fontWeight: 500 }}>7th Day of Training</span>
          </Text>
          <Title level={2} style={{ margin: "0 0 4px", fontWeight: 700, color: "#111827" }}>
            7th Day of Training
          </Title>
          <Text style={{ fontSize: 13, color: "#6b7280" }}>
            Record and track final-day training activities with media documentation.
          </Text>
        </div>
        <Button
          type="primary"
          ...
          onClick={() => navigate("/seventh-day-training-form")}
        >
          Add Record
        </Button>
      </div>
