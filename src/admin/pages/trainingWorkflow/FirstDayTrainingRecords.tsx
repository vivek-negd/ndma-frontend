...
/* ── Main component ── */
export const FirstDayTrainingRecords: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await CommonService.getSessionHistory({ day: 1 });
      console.log('📥 API Response for Day 1 Session History:', res);
      ...
      console.log('📋 Parsed Day 1 session history list:', list);
      const rows: Record[] = [];
      ...
      list.forEach((schedule: any) => {
        ...
        // Filter for Day 1 session only (backend might return all sessions)
        const day1Session = sessions.find((s: any) => 
          s.day_label?.toLowerCase() === 'day 1' ||
          s.day_label?.toLowerCase() === 'day1'
        );
        
        if (!day1Session) {
          console.log(`   ❌ No Day 1 session found for batch ${schedule.batch_no}`);
          return;
        }
        
        console.log(`   ✅ Found Day 1 session: ID=${day1Session.id}, date=${day1Session.date}`);
        
        rows.push({
          key: `${schedule.id}-${day1Session.id}`,
          ...
          date: day1Session.date || '',
          media: Array.isArray(day1Session.media_files) ? day1Session.media_files : [],
      ...
      console.log(`\n✅ Final Day 1 Records: ${rows.length} found\n`);
      setRecords(rows);
    } catch (err) {
  ...
  return (
    <div style={{ width: "100%", background: "#f3f4f6", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <Text style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 4 }}>
            Training Workflow /&nbsp;
            <span style={{ color: "#2563eb", fontWeight: 500 }}>1st Day of Training</span>
          </Text>
          <Title level={2} style={{ margin: "0 0 4px", fontWeight: 700, color: "#111827" }}>
            1st Day of Training
          </Title>
          <Text style={{ fontSize: 13, color: "#6b7280" }}>
            Record and track first-day training activities with media documentation.
          </Text>
        </div>
        <Button
          type="primary"
          ...
          onClick={() => navigate("/first-day-training-form")}
        >
          Add Record
        </Button>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
        ...
        <StatCard label="Total Volunteers Trained" value={totalVolunteers} color="#d97706" />
