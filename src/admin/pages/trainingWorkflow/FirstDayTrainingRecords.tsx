...
export const FirstDayTrainingRecords: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await CommonService.getTrainingSchedules(1);
      console.log('📥 API Response for Day 1:', res);
      ...
      list.forEach((sch: any) => {
        ...
        if (sessions.length === 1) {
          ...
          // Multiple sessions: filter strictly by day 1
          ...
          session = sessions.find((s: any, idx: number) => getSessionDay(s, idx) === 1);
          if (session) {
            console.log(`   ✅ Filtered to Day 1 session: ID=${session.id}`);
          } else {
            console.log(`   ❌ No Day 1 session found`);
          }
        }
