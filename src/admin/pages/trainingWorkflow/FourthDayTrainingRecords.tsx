...
export const FourthDayTrainingRecords: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await CommonService.getSessionHistory({ day: 4 });
      console.log('📥 API Response for Day 4 Session History:', res);
      ...
      console.log('📋 Parsed Day 4 session history list:', list);
      const rows: Record[] = [];
      ...
      list.forEach((schedule: any) => {
        ...
        // Filter for Day 4 session only (backend might return all sessions)
        const day4Session = sessions.find((s: any) => 
          s.day_label?.toLowerCase() === 'day 4' ||
          s.day_label?.toLowerCase() === 'day4'
        );
